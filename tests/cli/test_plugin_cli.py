...
from esgpull.cli.plugins import plugins, list_plugins, enable_plugin_cmd, disable_plugin_cmd
from esgpull.cli.plugins import config_plugin, test_plugin, show_signatures, create_plugin
from esgpull.plugin import PluginManager, Event
...
def test_plugins_create_command(tmp_path):
    ...
    assert "handle_query_updated" not in content  # Should only include specified events
...
def test_plugins_config_command(tmp_path, assets_path):
    ...
    result = runner.invoke(config_plugin, ["sample_plugin.log_level", "--default"])
    assert result.exit_code == 0
